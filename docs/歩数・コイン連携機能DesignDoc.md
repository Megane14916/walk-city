...
### Phase 3: Hook と状態整合性（Phase 4の前提として完了）

1. `useStepSync()` を実装した。
2. 実行中Promiseの再利用による二重送信防止と、API変更・アンマウント後のstale response対策を追加した。
3. 成功値を保持したまま後続エラーを表示し、再試行できる状態管理を追加した。
4. `useTownOverview()` に `applyStepSyncResult()` を追加し、サーバーの`coinBalance`で残高を置換するようにした。
5. 同期中は購入確定、購入送信中は同期を無効にした。
6. `CONFLICT` 時にTownを再取得するようにした。

### Phase 4: UI（実装完了）

1. `TownOverview.tsx` のダッシュボード領域に44 px以上の同期ボタンを追加した。
2. 初期歩数、同期後歩数、未連携、再連携が必要な状態を表示するようにした。
3. Health未連携では接続導線、権限不足では再連携導線、認証切れでは再ログイン導線を表示するようにした。
4. 同期中、獲得成功、差分歩数のみ、0件同期、同期失敗、再試行の表示を追加した。
5. 同期成功時に`steps`と`coinBalance`を同じレスポンスから反映するようにした。
6. 公開街では歩数、コイン、同期操作を非表示にし、`syncSteps()`を呼ばないようにした。
7. 既存の横スクロール可能なヘッダー内へ配置し、最小タップ領域とlive regionを実装した。

Phase 4 完了時の検証結果:

| コマンド・確認 | 結果 |
|---|---|
| Hook／Town UI対象テスト | 2ファイル、18件成功 |
| `npm test` | 15ファイル、125件すべて成功 |
| `npm run lint` | 成功 |
| `npm run build` | 成功 |
