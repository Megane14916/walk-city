...
sequenceDiagram
    ...
    Note over Edge,DB: 基本報酬 + 商業施設/工場効果<br/>台帳キーで二重付与防止
    DB-->>Edge: 歩数・獲得コイン・残高
    Edge-->>FE: 同期結果
    FE-->>User: 最新状態を表示
```
...
- 回転は不可。
- 移動は可能。
- 土地開放はコイン、アイテム、合計歩数のいずれか。採用方式は TBD。
- 障害物と建物削除は TBD。
- 道路は周辺建築を許可するが、周辺の定義は TBD。
...
| 公園（小） | 1×1 | 現時点なし |
| 病院 | 2×2 | 現時点なし |
| 商業施設 | 1×1 | 歩数で得るコインを増加。値 TBD |
| 農場 | 2×2 | 人口 +5 |
| 道路 | 1×1 | 周辺への建築を許可 |
| 役所 | 2×2 | 住宅（小）1軒あたりの人口 +20。範囲・重複 TBD |
| 工場 | 2×2 | 歩数で得るコインを増加。値 TBD |

コストはすべて TBD。効果は `building_effects` でデータとして管理する。

住宅（小）の基礎人口は10人。役所の効果対象となる住宅（小）は、1軒あたりさらに20人増加する。役所の適用範囲と複数設置時の重複ルールは TBD とする。
