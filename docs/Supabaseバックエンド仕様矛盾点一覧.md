...
| 道路一括配置 | `place_road_line`を正式APIとする | 文書上は解消。既存道路重複時の実装が契約と不一致 |
| 建物効果 | 住宅（小）+10、住宅（大）+50だけ | 文書へ反映済み。道路隣接・橋はマップルールとして維持 |
| 工場 | 効果なし | 解消 |
...
### `DEC-01` 住宅以外の「機能なし」に道路・橋を含めるか

ユーザー決定は「住宅（小）と住宅（大）だけが機能を持ち、その他の建物は存在するが機能を持たない」である。決定前の文書には次が残っていた。

- `commercial`: `step_coin_bonus_flat +50`
- `road`: `enables_adjacent_construction`
- 道路線が川を直交横断すると橋へ変換される配置ルール
- 道路が隣接しない通常建物は配置できないというルール

商業施設の+50は、今回の決定に従えば削除対象である。道路・橋だけは、`building_effects`ではなくマップの基本ルールとして残す余地があるため、範囲を明示する必要がある。

決定: `B`。`building_effects`は住宅2種だけとし、商業効果は廃止する。道路隣接と橋は建物効果ではなくマップルールとして残す。

選択肢:

- `A`: 住宅2種以外は完全に装飾とし、商業効果、道路隣接、橋変換をすべて廃止する。
- `B`: `building_effects`は住宅2種だけにし、商業効果は廃止するが、道路隣接と橋はマップルールとして残す。
- `C`: 商業施設と道路の現行効果も例外として残す。
...
住宅2種以外の効果を削除しても、住宅の説明文をDBで管理するかServiceで生成するかは残る。
...
住宅以外のコイン効果を廃止する場合、当面の`appliedBonuses`は常に空配列になる。

決定: `A`。API後方互換と将来拡張のためレスポンス型に残し、MVPでは常に`appliedBonuses: []`を返す。

選択肢:

- `A`: API後方互換と将来拡張のため`appliedBonuses: []`を残す。
- `B`: API・フロント型からフィールドを削除する。
...
| `FIX-08` | `place_building`は増分加算、`move_building`は人口再計算なし | 住宅2種だけを集計する共通人口再計算へ統一する |
| `FIX-09` | 通常道路の一括配置で既存道路をskipする | 契約どおり全体`CELL_OCCUPIED`にするか、仕様を変更する |
...
```text
DEC-01=B  # 住宅だけbuilding_effectsを持ち、道路・橋はマップルールとして残す
...
DEC-12=A  # appliedBonusesを空配列で残す
