...
共通 `types/common.ts` と `types/user.ts`、Town 機能の `features/town/`、`mocks/data/`、`mocks/services/` は最新の main で追加済みである。ランキング実装はこの構成を踏襲し、API 契約を `features/ranking/api/` に置く。`types/common.ts` の `ApiErrorCode`、`ApiError`、`ApiResult` と `types/user.ts` の `UserSummary` を重複定義しない。手順3で Vitest、手順4で Testing Library と jsdom を追加済みである。
...
`/town/:userId` の完全な画面実装は Map 機能の責務とする。ランキング項目は `/town/:userId` へ直接遷移する。`/users/:userId` の仮画面は用意しない（[ユーザーページDesignDoc.md](./ユーザーページDesignDoc.md)の決定により実装しない）。`/town/:userId` は Map 機能側の後続実装とする。
...
- 行全体または明示的なリンクで `/town/:userId` へ遷移できる。
...
| `/town/:userId` | `TownPage` | 必須 | ランキング項目の遷移先。街の表示は Map 機能の責務 |
| `/login` | `LoginPage` | 不要 | 未認証時の遷移先 |

`/users/:userId`（`UserPage`）は実装しない。対象ユーザーの街を確認する手段は `/town/:userId` に一本化する（[ユーザーページDesignDoc.md](./ユーザーページDesignDoc.md)）。手順6時点では `TownPage` が未実装のため、`/town/:userId` はまだルート登録しない。既存の認証・Health デモを `/login` に割り当て、`/` は暫定的に `/login` へ置換遷移する。Town 機能の画面実装時に、上位設計どおり `/` を認証必須の自分の街へ置き換える。
...
- `RankingItem` 内で `'/town/' + userId` を手書きしない。
...
具体的なテストツールは Vitest、React Testing Library、jsdom を採用する。手順3で Vitest と `test` scriptを追加してモック API 契約テストを実装し、手順4で Testing Library と jsdom を追加して Hook テストを実装済みである。同じ環境を手順5のコンポーネントテストでも使用する。
...
- ランキング項目は `/town/:userId` へ直接遷移する。`/users/:userId` の仮画面は用意しない。
- `/town/:userId` の画面は後続の Map 機能で実装する。
