...
| `small_park` | 公園 | 1×1 | 150 | true | 隣接する住宅（小）ごとに人口 +5、住宅（大）ごとに人口 +10（最大+40） | 価格は現行値 |
| `hospital` | 病院 | 2×2 | 600 | true | 町内の住宅（小）ごとに人口 +5、住宅（大）ごとに人口 +10（1軒分のみ、配置順不問） | 価格は現行値 |
...
| `town_hall` | 役所 | 2×2 | 3000 | true | 町内の住宅（小）ごとに人口 +20、住宅（大）ごとに人口 +30（1軒分のみ、配置順不問） | 価格は現行値 |
