-- リモートで欠落していた health_connections テーブルと sync_step_rewards 関数を復元する。
-- daily_step_records_user_date_source_key 制約はリモートに既に存在するため、ここでは追加しない。
...
begin
  if not exists (
    ...
    where conname = 'health_connections_user_id_fkey'
      and conrelid = 'public.health_connections'::regclass
  ) then
    alter table "public"."health_connections"
