...
export type MoveBuildingControlsProps = {
...
const invalidReasonMessages: Record<PreviewInvalidReason, string> = {
  ...
  CATALOG_ITEM_DISABLED: 'この建物は現在移動できません。',
...
function previewMessage(
  ...
  if (!anchor || !preview) return 'マップ上の移動先を選んでください。'
  if (preview.status === 'valid') return 'この位置へ移動できます。'
  if (preview.status === 'unknown') return preview.message
  return invalidReasonMessages[preview.reason]
}

export function MoveBuildingControls({
  item,
  displayName,
  anchor,
  preview,
  ...
  return (
    <section
      ...
      aria-labelledby="move-building-title"
    >
      <div className="flex items-center gap-3">
        <span
          className="grid h-11 w-11 shrink-0 place-items-center rounded-[13px] bg-[#dceee6] text-xl font-black text-[#245f51]"
          aria-hidden="true"
        >
          ↗
        </span>
        <div className="min-w-0 flex-1">
          <span className="text-[8px] font-black tracking-[.14em] text-[#548274]">
            移動モード
          </span>
          <h2
            className="m-0 truncate text-[15px] tracking-[-.02em] text-[#193b38]"
            id="move-building-title"
          >
            {displayName}を移動
          </h2>
        </div>
